...
const Dashboard = () => {
  ...
  //function to check is a date is in range 
...
//to filter using date and time
  ...
  //update the guage  when time frame changes
  useEffect(() => {
    const maxValues = {
  ...
  }, [currentTimeFrameData, timeFrame]); // the graph will be fillaccording to this date
  ...
  const displayedExpense = showAllExpense 
    ...
    const handleAddTransaction = async () => {
      ...
      try {
        ...
        console.error("Faild to add transactions", err?.response || err.message || err);
